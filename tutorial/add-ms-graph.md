...
Empiece extendiendo la `GraphHelper` clase para agregar una función para obtener los eventos del usuario. Abra el archivo **GraphHelper** y agregue las siguientes `import` instrucciones en la parte superior del archivo.
...
Agregue las siguientes funciones a la `GraphHelper` clase.
...
Ahora actualice `CalendarFragment` para usar estas nuevas funciones. Agregue las siguientes `import` instrucciones en la parte superior del archivo **CalendarFragment** .
...
Agregue los siguientes miembros a la `CalendarFragment` clase.
...
Ahora, agregue las siguientes funciones a `CalendarFragment` la clase para ocultar y mostrar la barra de progreso, y para proporcionar una devolución `getEvents` de llamada `GraphHelper`para la función en.
...
Por último, reemplace `onCreate` la función de `GraphHelper` la clase para obtener los eventos del usuario de Microsoft Graph.
...
Ahora puede reemplazar el volcado de JSON con algo para mostrar los resultados de forma fácil de uso. Empiece por reemplazar `TextView` en **App/res/layout/fragment_calendar. XML** por un. `ListView`
...
A continuación, cree un diseño para cada elemento en `ListView`el. Haga clic con el botón secundario en la carpeta **App/res/layout** , seleccione **nuevo**y, a continuación, **archivo de recursos de distribución**. Asigne un nombre `event_list_item`al archivo, cambie el **elemento raíz** a `RelativeLayout`y elija **Aceptar**. Abra el archivo **event_list_item. XML** y reemplace su contenido por lo siguiente.
...
Ahora, cree un adaptador de lista personalizado `ListView`para el. Esto es necesario para crear las vistas para los elementos de la lista y para asignar los campos de cada uno `Event` de ellos al `TextView` correspondiente en la vista.

Haga clic con el botón derecho en la carpeta **app/java/com. example. graphtutorial** y seleccione **nuevo**y, a continuación, **clase Java**. Asigne un nombre `EventListAdapter` a la clase y elija **Aceptar**. Abra el archivo **EventListAdapter** y reemplace el contenido por lo siguiente.
...
Por último, actualice `CalendarFragment` la clase para que `EventListAdapter` use el para `ListView`inicializar. Abra la clase **CalendarFragment** y agregue la siguiente función a la clase.
...
Agregue la siguiente línea de código en la `success` invalidación después `mEventList = iEventCollectionPage.getCurrentPage();` de la línea.

```java
addEventsToList();
```

Ejecute la aplicación, inicie sesión y pulse el elemento de navegación **calendario** . Debe ver la lista de eventos.
